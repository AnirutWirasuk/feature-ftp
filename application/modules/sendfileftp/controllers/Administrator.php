...
class Administrator extends MX_Controller {
	...
	public function main(){
		...
		$condition = array();
		$condition['fide'] = "ad_id,ad_fullname,position_name,ad_email,ad_editby,ad_lastedit,ad_lastlogin";
		$condition['where'] = array('ad_status' => 1);
	...
	public function form($id = ""){
		...
		if(!empty($id)){
			$condition = array();
			$condition['fide'] = "*";
			$condition['where'] = array('ad_id' => $id, 'ad_status' => 1);
	...
	public function create(){
		...
		if($this->tokens->verify('formcrf')){
			$data = array(
				...
				'ad_address' => $this->input->post('Text_Address'),
				'ad_tel' => $this->input->post('Text_Tel'),
				...
				'ad_editby' => $this->encryption->decrypt($this->input->cookie('sysn')),
				'ad_lastedit' => date('Y-m-d H:i:s'),
				'ad_status' => 1,
				'ad_datecreate' => date('Y-m-d H:i:s')
			);
	...
	public function checkemail(){
		...
		if(!empty($Text_Email)){
			...
			$condition['where'] = array('ad_email' => $Text_Email, 'ad_status' => 1);
